...
class APRPointCloudDataset(PointCloudDataset):
    """Class to handle APRPointCloud dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'APRPointCloud')
        ...
        # Dataset folder
        self.path = './data/APRPointCloud/train'
        ...
        if self.set == 'test':
            self.path = './data/APRPointCloud/test'
...
class APRPointCloudSampler(Sampler):
    """Sampler for S3DIS"""

    def __init__(self, dataset: APRPointCloudDataset):
...
class APRPointCloudCustomBatch:
...
def APRPointCloudCollate(batch_data):
    return APRPointCloudCustomBatch(batch_data)
