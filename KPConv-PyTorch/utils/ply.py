...
def read_tiff(filename, labelname):
    ...
    image_file = imread(filename)#.astype('float32')
    # v_min = image_file.min(axis=(1, 2), keepdims=True)
    # v_max = image_file.max(axis=(1, 2), keepdims=True)
    # image_file = ((image_file - v_min)/(v_max - v_min) * 255)
    image_file = resize_3d_volume(image_file, np.array([IMAGE_DEPTH, IMAGE_WIDTH, IMAGE_HEIGHT]))
    point_cloud = convert_to_pointcloud(image_file)
